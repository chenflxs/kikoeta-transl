...
- 输出支持目标语言单语、译文在前的双语、原文在前的双语，可选择 `LRC` 或 `SRT`；kikoeta 发起的任务可按曲目缓存 LRC，也可在“输出”页开启同步上传到 Kikoeta-LLS。
...
若要同步到 Kikoeta-LLS，先在 LLS 管理页设置独立的上传账号密码或 12 位密钥，再到本应用“输出”页开启“来自 Kikoeta 的翻译请求，产物同步上传到 Kikoeta-LLS”，选择验证方式并填入相同凭据。LLS 地址留空时使用本机 `http://127.0.0.1:2378`；跨机器时填写 LLS 的 HTTP/HTTPS 地址。只有来自 Kikoeta、成功完成且携带 RJ/VJ/BJ 作品号的任务会上传；结果会标记为 AI 歌词。上传失败会写入任务日志，本地导出和缓存仍保留。
