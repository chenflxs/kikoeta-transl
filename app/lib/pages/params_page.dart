...
class _ParamsPageState extends State<ParamsPage> {
  ...
  String _generatedTemplate() {
    final args = <String>[
      ...
      '--threads',
      field('threads').text.trim(),
      '--processors',
      field('processors').text.trim(),
      '--offset-t',
      field('offset_t').text.trim(),
      '--offset-n',
      field('offset_n').text.trim(),
      '--duration',
      field('duration').text.trim(),
      '--max-context',
      field('max_context').text.trim(),
      '--max-len',
      field('max_len').text.trim(),
      '--max-new-tokens',
      ...
      '--best-of',
      field('best_of').text.trim(),
      '--beam-size',
      field('beam_size').text.trim(),
      '--audio-ctx',
      field('audio_ctx').text.trim(),
      '--word-thold',
      field('word_thold').text.trim(),
      '--entropy-thold',
      field('entropy_thold').text.trim(),
      '--logprob-thold',
      field('logprob_thold').text.trim(),
      '--no-speech-thold',
      field('no_speech_thold').text.trim(),
      '--sensitivity',
      field('sensitivity').text.trim(),
      '--seed',
      field('seed').text.trim(),
      '--temperature-inc',
      field('temperature_inc').text.trim(),
      '--chunk-seconds',
      field('chunk_seconds').text.trim(),
      '--chunk-overlap',
      field('chunk_overlap').text.trim(),
      if (field('hotwords').text.trim().isNotEmpty) ...[
        '--hotwords',
        _quote(field('hotwords').text.trim()),
      ],
      if (splitOnWord) '--split-on-word',
      if (noFallback) '--no-fallback',
      if (noPunctuation) '--no-punctuation',
      if (field('punc_model').text.trim().isNotEmpty) ...[
        '--punc-model',
        field('punc_model').text.trim(),
      ],
      if (field('truecase_model').text.trim().isNotEmpty) ...[
        '--truecase-model',
        field('truecase_model').text.trim(),
      ],
      if (field('flush_after').text.trim() != '0') ...[
        '--flush-after',
      ...
      if (noGpu) ...[
        ...
        '--device',
        field('device').text.trim(),
        '--gpu-backend',
        field('gpu_backend').text.trim(),
      ],
      if (flashAttn) '--flash-attn' else '--no-flash-attn',
