...
def main() -> None:
    ...
    uses_local_llama = (
        (flags.enable_translate and settings.translate.provider == "local_llama")
        or (
            flags.enable_correct
            and (
                settings.correct.provider == "local_llama"
                or settings.translate.provider == "local_llama"
            )
        )
    )
